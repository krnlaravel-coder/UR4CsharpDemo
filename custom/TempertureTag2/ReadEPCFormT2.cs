...
namespace UHFAPP
{
    public partial class ReadEPCFormT2 : BaseForm
    {
        bool isPz = false;
        ...
        public ReadEPCFormT2()
        {
            InitializeComponent();
        }
        public ReadEPCFormT2(bool isOpen)
        {
            InitializeComponent();
            if (isOpen)
            {
        ...
        public ReadEPCFormT2(bool isOpen, TempertureTag2MainForm mainform)
        {
            InitializeComponent();
            if (isOpen)
            {
        ...
        private void UpdataEPC(string epc, string tid, string rssi, string count,string ant,string user)
        {
            ...
            if (exist[0])
            {
                ...
                epcList.Insert(index, new EpcInfo(epc,tid, int.Parse(count), DataConvert.HexStringToByteArray(epc), DataConvert.HexStringToByteArray(tid)));
        ...
        {
            string path = System.Environment.CurrentDirectory + "\\uhfData" + DateTime.Now.ToString("yyyy_MM_dd_HHssmm") + ".xls";
