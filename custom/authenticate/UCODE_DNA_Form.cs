...
{
    public partial class UCODE_DNA_Form : BaseForm
    {

        AuthenticateAPI api = new AuthenticateAPI();

        public UCODE_DNA_Form()
        {
            InitializeComponent();
            rbKey0.Checked = true;
            rbKeyID00.Checked = true;
        }
        /// <summary>
        /// 激活
        /// </summary>
        ...
        private void btnRead_Click(object sender, EventArgs e)
        {
            ...
            byte[] pwd=new byte[8];
            string result = "";
            if (rbKeyID00.Checked)
            {
                  result = UHFAPI.getInstance().ReadData(pwd, filterBank, filterAddr, filterDataLen, filterData, 0x03, 0xC0, 8);
            }
            else if (rbKeyID01.Checked)
            {
                  result = UHFAPI.getInstance().ReadData(pwd, filterBank, filterAddr, filterDataLen, filterData, 0x03, 0xD0, 8);
        ...
        private void btnActivate_Click(object sender, EventArgs e)
        {
            ...
            if (rbKeyID00.Checked)
            {
                  result = UHFAPI.getInstance().WriteData(pwd, filterBank, filterAddr, filterDataLen, filterData, 0x03, 0xC8, 1, data);
            }
            else if (rbKeyID01.Checked)
            {
                  result = UHFAPI.getInstance().WriteData(pwd, filterBank, filterAddr, filterDataLen, filterData, 0x03, 0xD8, 1, data);
            }
        ...
        private void btnWrite_Click(object sender, EventArgs e)
        {
            ...
            if (rbKeyID00.Checked)
            {
                result = UHFAPI.getInstance().WriteData(pwd, filterBank, filterAddr, filterDataLen, filterData, 0x03, 0xC0, 8, DataConvert.HexStringToByteArray(key));
            }
            else if (rbKeyID01.Checked)
            {
                result = UHFAPI.getInstance().WriteData(pwd, filterBank, filterAddr, filterDataLen, filterData, 0x03, 0xD0, 8, DataConvert.HexStringToByteArray(key));
            }
        ...
        private void btnAuthenticate_Click(object sender, EventArgs e)
        {
            ...
            int filterDataLen = int.Parse(txtLen.Text);
            byte[] filterData = DataConvert.HexStringToByteArray(txtFilter_EPC.Text);
            byte keyID = rbKey0.Checked?(byte)0:(byte)1;// byte.Parse(txtAuthenticateKeyID.Text);
