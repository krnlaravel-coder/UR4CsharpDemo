    ...
    public partial class ReadEPCForm2 : BaseForm
    {
        ...
        private void ReadEPC()
        {
            try
            {
                ...
                while (isRuning)
                {
                    ...
                    string ant = "";
                    string user = "";
                    bool result = uhf.ReadTagFromBuffer(ref epc, ref tid, ref rssi, ref ant);
                    if (result)
                    {
                ...
                if (isPz)
                {
                    ...
                    for (int k = 0; (k < 2) || result; k++)
                    {
                        ...
                        string ant = "";
                        string user = "";
                        result = uhf.ReadTagFromBuffer(ref epc, ref tid, ref rssi, ref ant);
                        if (result)
                        {
                            this.BeginInvoke(setTextCallback, new object[] { epc, tid, rssi, "1", ant });
        ...
        private void UpdataEPC(string epc, string tid, string rssi, string count,string ant)
        {
            ...
            if (exist[0])
            {
                ...
                lv.SubItems.Add(ant);
                lvEPC.Items.Add(lv);
                epcList.Add(new EpcInfo(epc,tid, int.Parse(count), DataConvert.HexStringToByteArray(epc), DataConvert.HexStringToByteArray(tid)));
