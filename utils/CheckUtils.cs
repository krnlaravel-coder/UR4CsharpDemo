...
namespace UHFAPP
{
    public class CheckUtils
    {
        ...
        public static int getInsertIndex(List<string> listEpc, string epc, bool[] exists)
        {
            ...
            {
                judgeIndex = (startIndex + endIndex) / 2;
                byte[] temp = DataConvert.HexStringToByteArray(listEpc[judgeIndex]) ;// System.Text.ASCIIEncoding.ASCII.GetBytes(listIp[judgeIndex].Replace(".", ""));
                ret = compareBytes(epcBytes, temp);
                if (ret > 0)
                {
                    if (judgeIndex == endIndex)
                    {
